...
#
# Wi-Fi power saving is also handled here. USB autosuspend lives in the [usb]
# plugin in tuned.conf. 802.11 power save has no sysfs path, so it remains in
# this profile script.
...
apply_power_policy() {
    echo quiet > "$PLATFORM_PROFILE"
    sleep 0.3

    ## MMIO
    echo 86000000 > "$RAPL_MMIO/constraint_2_power_limit_uw"
    echo 32000000 > "$RAPL_MMIO/constraint_1_power_limit_uw"
    echo 16000000 > "$RAPL_MMIO/constraint_0_power_limit_uw"
    echo 32000000 > "$RAPL_MMIO/constraint_0_time_window_us"

    ## MSR
    echo 86000000 > "$RAPL_MSR/constraint_2_power_limit_uw"
    echo 32000000 > "$RAPL_MSR/constraint_1_power_limit_uw"
    echo 16000000 > "$RAPL_MSR/constraint_0_power_limit_uw"
    echo 32000000 > "$RAPL_MSR/constraint_0_time_window_us"
}
